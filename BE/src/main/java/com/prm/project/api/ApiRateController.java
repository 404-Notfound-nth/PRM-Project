...
public class ApiRateController {
	...
	public ResponseEntity<Object> rateDoctor(@RequestBody RateDTO rateDTO){
		try {
			...
			return new ResponseEntity<Object>("Rate Successfull !!!", HttpStatus.CREATED);
		} catch (Exception e) {
	...
	public ResponseEntity<Object> getRateByDoctor(@PathVariable("doctorID") String doctorID){
		try {
			return new ResponseEntity<Object>( rateService.getRateByDoctor(doctorID),HttpStatus.CREATED);
		} catch (Exception e) {
			// TODO: handle exception
			return new ResponseEntity<Object>(HttpStatus.BAD_REQUEST);
		}
